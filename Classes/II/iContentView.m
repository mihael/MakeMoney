...
NSString *iContentViewURLNotification = @"iContentViewURLNotification";
...
- (void)handleButton:(id)sender
{
	...
	for (NSString *expression in expressions)
	{
		...
		while (match = [enumerator nextObject])
		{
			...
			{
				[[NSNotificationCenter defaultCenter] postNotificationName:iContentViewURLNotification object:match];
			}
		}
	}
}
...
- (void)setNumberOfLines:(NSInteger)_numberOfLines{
	[self.label setNumberOfLines:_numberOfLines];
}
...
- (void)forwardInvocation:(NSInvocation*)invocation
{
	...
	DebugLog(@"forwardInvocation: selector = %@", NSStringFromSelector(aSelector));
...
{
	DebugLog(@"methodSignatureForSelector: selector = %@", NSStringFromSelector(aSelector));
...
{
	DebugLog(@"respondsToSelector: selector = %@", NSStringFromSelector(aSelector));
